...
# Drafty — the review layer for AI-drafted work
...
| `drafty comments watch <slug> [--json] [--backlog]` | **Socket mode** — stream new human comments and mark that canvas attended, so the daemon defers; unlike a slug watch, `--live` is only a doorbell and claims nothing. |
